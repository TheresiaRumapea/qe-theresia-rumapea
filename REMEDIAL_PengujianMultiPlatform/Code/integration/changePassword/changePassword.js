import { Given, When, And, Then } from "cypress-cucumber-preprocessor/steps";
const blibiPage = require("../../pages/blibiPage");

Given('Navigate to login page', () => {
  cy.visit('/')
  cy.viewport(1920, 1080)
})
When('User enter email {string}', (id) => {
  blibiPage.typeEmail(id)
})
And('User enter password {string}', (password) => {
  blibiPage.typePassword(password)
})
Then('Click button login' , () => {
  blibiPage.clickButtonLogin()
})
Given('Navigate to open profile', () => {
  blibiPage.clickIconProfile()
})
When('Click account setting', () => {
  blibiPage.clickAccountSetting()
})
And('Click tab change password', () =>{
  blibiPage.clickChangePasswordTab()
})
And('User enter current password {string}', (currentPassword) => {
  blibiPage.typeCurrentPassword(currentPassword)
})
And('User enter password {string}', (password) => {
  blibiPage.typePassword(password)
})
And('User enter password confirmation {string}', (passwordConfirmation) => {
  blibiPage.typePasswordConfirmation(passwordConfirmation)
})
Then('Click button change password', () => {
  blibiPage.clickChangePasswordButton()
})
